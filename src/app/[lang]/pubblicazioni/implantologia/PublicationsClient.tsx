...
interface PublicationsDictionary {
  pubblicazioni: {
    implantologia: {
      title: string;
      goto: string
...
function PublicationsClient({ lang, dict }: PublicationsClientProps) {
  const [publications, setPublications] = useState<Publication[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`https://php.leone.it/api/GetPubblicazioni.php?lingua=${lang.toUpperCase()}`, {
      method: "GET",
      headers: {
        Authorization:
  ...
  return (
    <div className="container">
      <h1 className="blue font-bold py-5">{dict.pubblicazioni.implantologia.title}</h1>
      {loading ? (
        <div role="status">
          <svg
            aria-hidden="true"
            ...
            <li key={publication.id} className="publication-item">
              ...
              {publication.link !== "" && (
                <Link target="_blank" href={publication.link} className="link">
                  {dict.pubblicazioni.implantologia.goto}
                </Link>
              )}
            </li>
          )) : <h3>{dict.pubblicazioni.implantologia.notFound}</h3>}
        </ul>
      )}
    </div>
  );
