...
interface ShopEssenzaProps {
  lang: 'it' | 'en' | 'es';
}

export default function ShopEssenza({ lang }: ShopEssenzaProps) {
  ...
  return (
    <>
      ...
      >
        Essenza <span className="ms-3">{chevron}</span>
      </h1>
      <div style={{ display: isOpen ? 'block' : 'none' }}>
        <ShopContent lang={lang} idUser={userId} idGruppo="2" />
        </div>
    </>
  );
}